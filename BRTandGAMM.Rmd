...
**Question:** Why do two of the plots look so strange?

```{r GAMM statistics, eval=T, echo=F}
...
```{r GAMM_testtrain, eval=T, echo=F}
train <- d  %>%
    mutate(partition='train')
test <- d  %>%
    mutate(partition='test')
...
print(paste0("AUC value for test data is: ", saveAUC(newdata$presabs,newdata$fit)))
#print(saveAUC(newdata$presabs,newdata$fit))    
print("Reciever operator curve:")
plotROC(newdata$presabs,newdata$fit, colorize = TRUE)
...
You might be interested in perusing [resources on species distribution modeling](http://ucsb-bren.github.io/esm296-4f/wk4/) from week 4 of Ben Best's Advanced GIS course which used a different modeling technique, the generalized linear model (vs Maxent) and pseudo-absence points (vs background). 
© 2020 GitHub, Inc.