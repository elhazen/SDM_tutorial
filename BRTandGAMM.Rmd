...
# install packages below if don't already have them in your library
  #install.packages("rgdal")library(sp)
...
A common method to evaluate the fit of a model is **cross-validation**. In this approach you create a model with one *training* data set and then *testing* occurs using another data set of known occurences. Typically the test and training data sets are created through random sampling (without replacement) from a single data set.
